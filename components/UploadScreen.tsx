...
export function UploadScreen({ onBack, onSubmit }: { onBack: () => void; onSubmit: () => void }) {
      ...
      <CardContent className="p-6">
        <div className="flex flex-col items-center justify-center space-y-6">
          <div className="w-full flex justify-between items-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900">上傳對話截圖</h2>
            <div className="w-[64px]"></div> {/* 為了保持標題居中 */}
          </div>

          {selectedFile ? (
            <div className="relative w-full max-w-sm aspect-[3/4]">
              <Image src={selectedFile || "/placeholder.svg"} alt="Preview" fill className="object-cover rounded-lg" />
              <Button
                variant="outline"
                size="sm"
            ...
            <label className="flex flex-col items-center justify-center cursor-pointer w-full max-w-sm aspect-[3/4] rounded-lg bg-pink-50 border-2 border-dashed border-pink-200 hover:bg-pink-100 transition-colors duration-300">
              ...
              <span className="text-lg font-medium text-pink-600">點擊上傳對話截圖</span>
              <span className="text-sm text-gray-500 mt-1">或拖放圖片到此處</span>
              <input type="file" className="hidden" accept="image/*" onChange={handleFileChange} />
            </label>
          )}

          <div className="w-full space-y-4">
            <h3 className="text-lg font-semibold text-gray-900">選擇回覆語氣</h3>
            <RadioGroup
              defaultValue="casual"
              value={replyMode}
          ...
          <div className="w-full space-y-4">
            <h3 className="text-lg font-semibold text-gray-900">選擇親密程度</h3>
            <Slider
              defaultValue={[50]}
              max={100}
              step={1}
